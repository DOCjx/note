	...
	<div class="container">
		<form action="<?php echo U('Index/send');?>" method="post" accept-charset="utf-8" class="row">
			<table class="table table-striped table-hover col-md-4">
				...
				<?php if(is_array($users)): $i = 0; $__LIST__ = $users;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dataDetial): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($dataDetial['id']); ?>">
						<td>
							<input type="checkbox" name="userId[]" class="checkboxs" value="<?php echo ($dataDetial['id']); ?>">
						</td>
